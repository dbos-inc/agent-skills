...
- Cancellation happens at the **beginning of the next step** - the current step completes first, unless it observes `DBOS.stepStatus.cancelSignal` and stops early (see `step-timeouts.md`)
- When the timeout expires, the workflow **and all its child workflows** are cancelled
